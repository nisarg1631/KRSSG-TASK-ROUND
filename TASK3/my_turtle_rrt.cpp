...
void rrt_connect(point p_start, point p_end, Mat &img, Mat obst){
  ...
  while(connected!=1){
    ...
    if(state==1){
      ...
      line(img,Point((step_node->loc).x,(step_node->loc).y),Point((step_node->parent->loc).x,(step_node->parent->loc).y),Scalar(0,255,255),2,8);
      //imshow("IMAGE",img);
      //waitKey(10);
...
int main(int argc, char **argv){
    ...
    Mat img=imread("/home/mi_ni_/catkin_ws/src/myturtle/src/rrt_connect.png",1);
    Mat obst=imread("/home/mi_ni_/catkin_ws/src/myturtle/src/rrt_connect.png",0);
    ...
    ros::Subscriber current_pose_sub=n.subscribe("/turtle1/pose",100,current_position);
    ros::Subscriber obstacle_pose_sub=n.subscribe("/turtle2/pose",100,obstacle_position);
    ros::Publisher command_turtle = n.advertise<geometry_msgs::Twist>("/turtle1/cmd_vel", 100);
